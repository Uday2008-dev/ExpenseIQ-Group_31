# ExpenseIQ – Smart Expense Tracker
### Capstone Project – Group 31  
**Indian Institute of Technology Patna**

---

## 📌 Overview
ExpenseIQ is a **smart expense tracking system** designed to help users manage their finances with ease.  
The project leverages **Python (Flask/Django)** for backend services, **SQLite/PostgreSQL** for data storage, and provides a clean, intuitive interface for recording, categorizing, and analyzing expenses.
The project also leverages **HTML(Structure)**,**CSS(Design)** & **JavaScript(Logic in browser)** 
This project was developed as part of the **Capstone Project** by **Group 31, IIT Patna**.

---

## 🎯 Features
- **User Authentication** – Secure login & signup with password hashing.  
- **Expense Management** – Add, update, delete, and view expenses.  
- **Categorization** – Expenses grouped by categories (Food, Travel, Bills, etc.).  
- **Smart Insights** – Monthly/weekly reports, spending trends, and charts.  
- **Export Options** – Download reports in CSV/PDF format.  
- **Responsive UI** – Simple and mobile-friendly interface.  

---

## 🛠️ Tech Stack
- **Backend:** Python (Flask/Django)  
- **Database:** SQLite / PostgreSQL  
- **Frontend:** HTML, CSS, JavaScript (Bootstrap/React optional)  
- **Authentication:** JWT / Flask-Login  
- **Visualization:** Matplotlib / Plotly
 
 **To Run This Application**
 ```bash
   python app.py
```


**OPEN THE BROWSER**
  http://127.0.0.1:5000/

📊 Example Usage
- Add Expense: Enter amount, category, and description.
- View Report: Generate monthly spending charts.
- Export Data: Download expense history in CSV/PDF.


👨‍🎓 Contributors – Group 31
- Harshit Divyanhu – Backend Development & Database
- Uday Dubey – Frontend & UI Design 
- Harshit Divyanhu & Uday Dubey - Authenticationng & Visualization 
- Rohan Dubey – Reporting  And PowerPoint Presentation Works
- Shashank Dubey -Established connection between backend to frontend with database 
- Sanskar Dubey - He  contributed as database manager.



